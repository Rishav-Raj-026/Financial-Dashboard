...
st.markdown("""
    ...
    /* 3. AGGRESSIVE POINTER CURSOR FIXES */
    ...
    button, div[role="button"] { cursor: pointer !important; }
    div[data-baseweb="tag"] { cursor: pointer !important; }
...
with st.expander("📂 Upload Your Own File or Connect API", expanded=False):
    ...
    user_file = None; user_url = None; api_trigger = False
    with tab_up:
        user_file = st.file_uploader("Drop CSV/Excel here", type=['csv', 'xlsx', 'json'])
    with tab_api:
        c1, c2 = st.columns([3, 1])
        with c1: user_url = st.text_input("Enter API URL")
        with c2: 
...
if user_file:
    ...
    if os.path.exists('financial_data.csv'):
        ...
        st.warning("⚠️ 'financial_data.csv' not found. Please upload a file.")
...
if raw_data is not None:
    ...
    elif used_source == "Demo": st.toast("ℹ️ Using Demo Data")
    
    st.markdown('<div class="stCard">', unsafe_allow_html=True)
    
    # --- FORM START: Fixes jumpy date selection ---
    with st.form("filter_form"):
        col_date, col_curr, col_btn = st.columns([3, 1, 1])
        min_date = df['date'].min(); max_date = df['date'].max()
        
        with col_date:
            # We initialize with full range
        ...
        with col_btn:
            st.markdown('<div style="margin-top: 29px;"></div>', unsafe_allow_html=True)
            # This button is required to update the dashboard
            submitted = st.form_submit_button("APPLY FILTERS", use_container_width=True)

    # Filtering Logic: Only runs if range is valid
    if len(date_range) == 2:
        mask = (df['date'] >= pd.to_datetime(date_range[0])) & (df['date'] <= pd.to_datetime(date_range[1]))
        filtered_df = df.loc[mask]
    else:
        filtered_df = df # Fallback if only 1 date picked inside form (rare)
    ...
    with t1:
        ...
        if 'Asset Value' in filtered_df['type'].unique():
             line_df = filtered_df[filtered_df['type'] == 'Asset Value']; y_col = 'amount'; title = 'Asset Value'
        else:
    ...
    with t3:
        ...
        if extra_cols:
            ...
            selected_extras = st.multiselect("Select additional columns:", options=extra_cols, default=extra_cols)
